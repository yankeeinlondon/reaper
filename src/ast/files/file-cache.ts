...
import type { ReaperOpts } from "~/lib";
...
import { FILE_REF_PREFIX, getRoot } from "~/constants";
...
export function asFileRef(filepath: string | Diagnostic): FileRef {
    if (isString(filepath)) {
        ...
        if (p) {
            ...
            throw InvalidFilepath(`A diagnostic was passed into asFileRef() which could not be resolved to a FileRef`, { diagnostic: filepath },
            );
        }
    }
}
